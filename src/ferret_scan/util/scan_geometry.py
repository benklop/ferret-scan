...
def capture_precise_corners(steps=3):
    corners = []
    image_capture = runtime_engine.image_capture
    ...
    for i in range(steps):
        ...
        if image is not None:
            c = runtime_engine.image_detection.detect_corners(image)
            if c is not None:
...
def estimate_platform_angle_from_pattern(pose, use_camera_space=False):
    calibration = runtime_engine.platform_extrinsics.calibration_data
    if pose is not None:
        if (
            calibration.platform_rotation is not None
            and np.count_nonzero(calibration.platform_rotation) > 0
            and calibration.platform_translation is not None
            and np.count_nonzero(calibration.platform_translation) > 0
            and not use_camera_space
        ):
            ...
            vv = np.append(
                pc, [-calibration.platform_translation / np.linalg.norm(calibration.platform_translation)], axis=0
            )
...
def rotatePoint2Plane(A, n, d):
    calibration_data = runtime_engine.platform_extrinsics.calibration_data
    ...
    if discr >= 0:
        ...
        if np.linalg.norm(A4 / np.linalg.norm(A4) - AA1 / np.linalg.norm(AA1)) < np.linalg.norm(
            A4 / np.linalg.norm(A4) - AA2 / np.linalg.norm(AA2)
        ):
