...
class Cloud:
    ...
    def clear(self, length=0):
        self.points_xyz = np.array([0.0, 0.0, 0.0] * length)
        self.points_rt = None  # polar
        self.points_l = np.array([0.0] * length)  # point scan angle
    ...
    def add(self, points_xyz, points_l, points_color=np.array([0, 0, 0], dtype=np.uint8), points_rt=None):
        if len(points_xyz.shape) <= 1:
            ...
            print(f'Adding {len(points_l)} points')
            # add lists
            if self.points_rt is not None:
                if points_rt is None:
                    points_rt = cart2pol(points_xyz[:, [0, 1]])
...
class ChunksPolar:
    def __init__(self, width=2.0, height=2.0, maxvar=4.0, min_amount=3):
    ...
    def put_points(self, src_cloud):
        ...
        if points_rt is not None:
            ...
            for _id, (_t, _z) in enumerate(zip(t, z)):
                self.chunks.setdefault(_z, {}).setdefault(_t, []).append(_id)

            # calculate chunks parameters
            print('\tCalculating chunks')
            delete = []
            for _z, T in self.chunks.items():
                ...
                for _t, D in T.items():
                    ...
                    _cnt = len(D)
                    if _var[0] <= self.maxvar and _cnt >= self.min_amount:
                        ...
                        avg_rt = np.array(
                            [np.mean(points_rt[D][:, 0], axis=0), stats.circmean(points_rt[D][:, 1], axis=0)]
                        )
                ...
                if len(s) > 10:  # minimum amount of chunks for precise align ( minimum = 2 to solve equations )
                    self.chunks[_z] = s
                    print(f'\tChunk z={_z} - {len(s)}')
                else:
                    delete.append(_z)
            for _x in delete:
                del self.chunks[_x]
        print(f'[Done] build {self.chunks_count} chunks')

    def get_center_vertexes(self):
        res = np.empty((self.chunks_count, 3), dtype=np.float32)
        cnt = 0
        for _z, T in self.chunks.items():  # horizontal slices
            for _t, D in T.items():
    ...
    def intersect(self, chunksB):
        ...
        cnt = 0
        res = {}
        for _z, TA in self.chunks.items():  # horizontal slices
            TB = chunksB.chunks.get(_z, None)
            if TB is None:
                continue  # no matching chunk in B

            for _t, DA in TA.items():
                DB = TB.get(_t, None)
                if DB is None:
                    continue
    ...
    def fit_chunks(self, chunksB):
        ...
        for _z, TA in self.chunks.items():  # horizontal slices
            ...
            idxA = []
            for _t, D in TA.items():
                idxA.append(D[0])

            idxB = []
            for _t, D in TB.items():
                idxB.append(D[0])

            print(f'Layer {_z}: {len(idxA)} vs {len(idxB)} points')
            delta = fit_clouds(
                self.cloud.points_xyz[idxA][:, [0, 1]],
                self.cloud.Mrev[idxA],
                chunksB.cloud.points_xyz[idxB][:, [0, 1]],
                chunksB.cloud.Mrev[idxB],
                delta,
            )
            res += [delta.tolist() + [_z * self.height]]
            print(f'>>>>>>>>> {delta.tolist() + [_z * self.height]} <<<<<<<<<<<')

        return np.array(res)


class ChunksCubic:
    def __init__(self, width=2.0, height=2.0, min_amount=3):
        self.width = width
        self.height = height
        self.min_amount = min_amount
    ...
    def put_points(self, src_cloud):
        ...
        for _id, (_x, _y, _z) in enumerate(xyz):
            self.chunks.setdefault(_z, {}).setdefault(_y, {}).setdefault(_x, []).append(_id)

        # calculate chunks parameters
        print('\tCalculating chunks')
        delete = []
        for _z, Y in self.chunks.items():
            # T - current horizontal slice (Thetas list)
            yy = {}
            for _y, X in Y.items():
                xx = {}
    ...
    def get_center_vertexes(self):
        res = np.empty((self.chunks_count, 3), dtype=np.float32)
        cnt = 0
        for _z, Y in self.chunks.items():  # horizontal slices
            for _y, X in Y.items():
                for _x, D in X.items():
    ...
    def intersect(self, chunksB):
        print(f'Intersect chunks {self.chunks_count} vs {chunksB.chunks_count}')
        buf = [0, 0] * min(self.chunks_count, chunksB.chunks_count)
        cnt = 0
        res = {}
        for _z, YA in self.chunks.items():  # horizontal slices
            YB = chunksB.chunks.get(_z, None)
            if YB is None:
                continue  # no matching chunk in B

            for _y, XA in YA.items():
                XB = YB.get(_y, None)
                if XB is None:
                    continue

                for _x, DA in XA.items():
                    DB = XB.get(_x, None)
                    if DB is None:
                        continue
...
class MeshTools:
    ...
    def reconstruct_slices(self, step=None):
        ...
        logger.info(f'{cur_slice} Slices reconstructed. Angle: {np.rad2deg(step)} deg')
...
class CloudTools:
    ...
    def unwrap_mesh(self, width=360.0, scale_z=1.0):
        if self.mesh is None:
            return
        ...
        if self.radial is not None:
            vertexes = np.array(
                list(zip(self.radial[:, 0], (self.radial[:, 1]) * width / 2 / np.pi, self.vertexes[:, 2] * scale_z))
            )
    ...
    def flatten_mesh(self, width=360.0, scale_z=1.0):
        if self.mesh is None:
            return
        ...
        slice_angle = np.array(self.vertexes_meta[:, 1].tolist(), dtype=np.float32)[:, 1]  # angle
        # slice_angle = slice_angle*0xFF/2/np.pi
        # col = np.array( [ slice_angle, slice_angle, slice_angle ], dtype=np.uint8).T

        if self.radial is not None:
            x = self.radial[:, 0] * np.cos(self.radial[:, 1] + slice_angle)
            y = self.radial[:, 0] * np.sin(self.radial[:, 1] + slice_angle)
    ...
    def get_corrected_vertices(self, delta=[0, 0]):
        ...
        print(f'Get corrected {delta}, {len(self.radial)} points')

        slice_angle = np.array(self.vertexes_meta[:, 1].tolist(), dtype=np.float32)[:, 1]  # angle
        res = np.copy(self.radial)  # keep original data intact
        res[:, 1] += slice_angle
        res = pol2cart(res)
        res[:] += np.array(delta)
        res = cart2pol(res)
    ...
    def build_chunks(self, width=2.0, height=2.0, maxvar=4.0, min_amount=3):
        print('Build chunks')
        if self.radial is None:
            self.make_radial()
        ...
        if self.radial is not None:
            ...
            for _id, (_r, _t, _z, _c, _m) in enumerate(
                zip(rad, t, z, self.colors, self.vertexes_meta)
            ):  # radial, chunk_theta, chunk_z, color, laser num
                self.chunks.setdefault(_m[0], {'width': width, 'height': height}).setdefault(_z, {}).setdefault(
                    _t, []
                ).append([_id, _r, np.array(_c, np.uint16), _m[1][1]])  # id, [radial], [color], slice_l

            # calculate chunks parameters
            print('Calculating chunks')
            for _l, C in self.chunks.items():
                # C - chunk for current laser cloud
                delete = []
                for _z, T in C.items():
                    # T - current horizontal slice (Thetas list)
                    # print "T: {0}:{1}".format(_z,T)
                    if not isinstance(_z, int):
                        continue  # metadata
                    ...
                    for _t, D in T.items():
                        ...
                        _cnt = len(D)
                        if _var[0] <= maxvar and _cnt >= min_amount:
                            if _var[1] > np.deg2rad(10):
                                print(f'Out points {np.rad2deg(np.array(D[:, 1].tolist())[:, 1])}')
    ...
    def get_chunk_vertexes(self, chunk, delta=[0, 0]):
        ...
        np.deg2rad(chunk['width'])
        height = chunk['height']
        vertexes = []
        colors = []
        for _z, T in chunk.items():
            # T - current horizontal slice
            if isinstance(_z, int):
    ...
    def adjust_chunks(self, chunkA, chunkB):
        ...
        assert chunkA['width'] == chunkB['width'] and chunkA['height'] == chunkB['height'], (
        ...
        np.array(self.vertexes_meta[:, 1].tolist())[:, 1]  # turntable L
        delta = [0]  # [0,0]
        for _z, TA in chunkA.items():  # horizontal slices
            if not isinstance(_z, int):
            ...
            for _t, DA in TA.items():
                ...
                chord = 2 * np.tan(np.abs(DA[2] - DB[2]) / 2)
...
# ----------------- R Mat -------------
def rmat2d_arr(radians):
    # radians - array of radians
    c, s = np.cos(radians), np.sin(radians)
    # return np.matrix([[c, -s], [s, c]])
    return np.array([c, -s, s, c]).T.reshape((-1, 2, 2))


def rmat3d_arr(radians):
    c, s = np.cos(radians), np.sin(radians)
    return np.array([c, -s, 0, s, c, 0, 0, 0, 1]).T.reshape((-1, 3, 3))
...
def apply_mat_arr(mat, arr):
    ...
    assert mat.shape[1:] == (s, s), f'N x [{s}x{s}] matrices required'
...
def fit_correction(pA, pB, lAB, prev=[0]):  # [0,0]):
    ...
    # print "Fit A {0}, B {1}, lAB {2}".format(len(pA),len(pB),len(lAB))
    offset, ier = optimize.leastsq(risiduals_fit_correction, V, args=((pA, pB, lAB)))
    print(f'Fit result: {offset}  ier={ier}  delta_r={np.mean(risiduals_fit_correction(offset, pA, pB, lAB))}')
    # print np.round( np.linalg.norm( pA[:], axis=1 ), 3)
    # print np.round( np.linalg.norm( pA[:] + offset, axis=1 ), 3)
    # print np.round( np.linalg.norm( pB[:] + offset, axis=1 ), 3)
    # print np.round( risiduals_fit_correction(offset,pA,pB), 3)
...
def fit_clouds(PA, MAneg, PB, MBneg, prev=np.array([0.0, 0.0])):
    V = prev
    offset, ier = optimize.leastsq(risiduals_fit_clouds, V, args=((PA, MAneg, PB, MBneg)))
    print(f'Fit result: {offset}  ier={ier}')
