...
from ferret_scan.gui.util.augmented_view import (
    augmented_draw_lasers_on_pattern,
    augmented_draw_lasers_on_platform,
    augmented_draw_platform,
)
...
from ferret_scan.util.gryphon_util import rotatePoint2Plane
                ...
                if pose is not None:
                    for i, laser in enumerate(calibration_data.laser_planes):
                        if not laser.is_empty():
                            # print(i)
                            laser_point = rotatePoint2Plane(pose[1].T[0], laser.normal, laser.distance)
                            # print(laser_point)
                            if laser_point is not None:
                                cv2.putText(
                                    image,
                                    str(i) + ': ' + str(np.round(laser_point, 2)),
                                    (10, 80 + i * 30),
                    ...
                    cv2.putText(
                        image,
                        'R: ' + str(np.round(np.linalg.norm(A[0:2]), 2)),
                        (10, 160),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        1.0,
                        (255, 0, 0),
