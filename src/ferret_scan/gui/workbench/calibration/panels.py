...
from ferret_scan.gui.util.gryphon_controls import Header
from ferret_scan.gui.widgets import (
    Button,
    CheckBox,
    ComboBox,
    ExpandablePanel,
...
class CameraIntrinsics(ExpandablePanel):
    ...
    def _update_new_camera_matrix(self, value):
        if driver.is_connected:
            ...
            ruler_len = profile.settings['new_camera_ruler']
            h = profile.settings['new_camera_distance_h']
            v = profile.settings['new_camera_distance_v']
            M = [[res[0] * h / ruler_len, 0, res[0] / 2], [0, res[1] * v / ruler_len, res[1] / 2], [0, 0, 1]]
