...
from ferret_scan.engine.driver.camera import InputOutputError
from ferret_scan.gui.workbench.scanning.diy_scan_panels import MeshCorrection, Photogrammetry, PointCloudColor
from ferret_scan.gui.workbench.scanning.panels import PointCloudROI, RotatingPlatform, ScanParameters
from ferret_scan.gui.workbench.scanning.view_page import ViewPage
...
class ScanningWorkbench(Workbench):
    ...
    def on_play_tool_clicked(self, event):
        if ciclop_scan._inactive:
            ...
            if profile.settings['texture_mode'] == 'Laser BG' and not profile.settings['remove_background_scanning']:
                dlg = wx.MessageDialog(
                    self,
                    _(
