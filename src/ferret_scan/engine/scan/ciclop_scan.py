...
from ferret_scan.engine.calibration.calibration_data import CalibrationData
from ferret_scan.engine.scan.current_video import CurrentVideo
from ferret_scan.engine.scan.scan import Scan, ScanError
from ferret_scan.engine.scan.scan_capture import ScanCapture
from ferret_scan.util import profile
from ferret_scan.util.scan_geometry import decode_color
...
class CiclopScan(Scan):
    ...
    def _initialize(self):
        ...
        if self.ph_save_enable:
            self.ph_save_folder = profile.settings['ph_save_folder'] + datetime.datetime.now().strftime(
                '/scan%Y-%m-%d_%H-%M'
            )
            ...
            if self._inactive:
                ...
                if abs(self._theta) >= 360.0:
                    ...
                    if self._debug and system == 'Linux':
                        ...
                        print(string_time + f' elapsed progress: {int(self._theta / 3.6)} %')
                        print(
                            string_time
                        ...
                        print(string_time + f' elapsed angle: {float(self._theta)}º')
                        print(string_time + f' capture: {int((self._end - begin) * 1000)} ms')
            # Sleep
    ...
    def _process(self):
        ...
        self._end = time.time()
        logger.info(
            'Finish scan {0} %  Time {1}'.format(
                progress, time.strftime('%M\' %S"', time.gmtime(self._end - self._begin))
            )
        )
    ...
    def _process_capture(self, capture):
        if _is_ferret_mode() and capture.depth is not None:
            ...
            if point_cloud is not None and self.point_cloud_callback:
                ...
                self.point_cloud_callback(
                    self._range, self._progress, (point_cloud, texture), (0, capture.count, capture.theta)
                )
            if self.ph_save_enable and capture.count % self.ph_save_divider == 0:
                filename = self.ph_save_folder + f'/img{np.rad2deg(capture.theta):03.03f}.png'
        ...
        for i in range(2):
            if capture.lasers[i] is not None:
                ...
                point_cloud = self.point_cloud_generation.compute_point_cloud(capture.theta, points_2d, i)
                # print("Processed: {0:f} - {1}".format(np.rad2deg(capture.theta),i))

                if self.point_cloud_callback:
                    self.point_cloud_callback(
                        self._range, self._progress, (point_cloud, texture), (i, capture.count, capture.theta)
                    )
