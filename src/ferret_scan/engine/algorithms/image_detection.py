...
from ferret_scan.gui.util.augmented_view import augmented_pattern_mask
    ...
    def detect_pose_from_corners(self, corners):
        if corners is not None:
            ret, rvecs, tvecs = cv2.solvePnP(
                pattern.object_points, corners, calibration_data.camera_matrix, calibration_data.distortion_vector
            )
            if ret:
    ...
    def _detect_chessboard(self, image, fast=True):
        if image is not None:
            if pattern.rows > 2 and pattern.columns > 2:
                ...
                ret, corners = cv2.findChessboardCorners(gray, (pattern.columns, pattern.rows), flags=fl)
                if ret:
