...
class DepthToPointCloud:
    ...
    def compute_point_cloud(self, theta, depth_mm, color_rgb=None, depth_scale=1.0):
        """Return (3xN model-frame points, 3xN uint8 texture) or (None, None).

        ``color_rgb`` is HxWx3 RGB (same layout as camera frames in ferret-scan).
        """
        if depth_mm is None:
            return None, None
        ...
        texture = self._sample_texture(color_rgb, depth_mm, points_cam)
    ...
    def _sample_texture(self, color_rgb, depth_mm, points_cam):
        ...
        tex[0, valid] = color_rgb[v[valid], u[valid], 0]
        tex[1, valid] = color_rgb[v[valid], u[valid], 1]
        tex[2, valid] = color_rgb[v[valid], u[valid], 2]
        return tex
