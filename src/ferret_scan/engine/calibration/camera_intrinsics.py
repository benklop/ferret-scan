...
class CameraIntrinsics(Calibration):
    ...
    def calibrate(self):
        ...
        ret, cmat, dvec, rvecs, tvecs = cv2.calibrateCamera(
            self.object_points, self.image_points, self.shape, cmat0, dvec0, flags=flags
        )
        ...
        if version_gt(cv2.__version__, '4.0.0'):
...
#                        criteria= (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 30,  2.2204460492503131e-16) ); # | CALIB_USE_LU
        ...
        if ret:
            ...
            for i in range(len(self.object_points)):
                imgpoints2, _ = cv2.projectPoints(self.object_points[i], rvecs[i], tvecs[i], cmat, dvec)
